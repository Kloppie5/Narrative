...
namespace Narrative {
  public static class MemoryHelper {
    ...
    public static Boolean Match( ProcessManager64 manager, UInt64 address, String pattern ) {
    ...
    public List<UInt64> FindPatternAddresses ( UInt64 start, UInt64 end, BytePattern pattern ) {
      ...
      while ( currentAddress < end ) {
        ...
        */
    public static T ReadRelative<T> ( ProcessManager64 manager, UInt64 offset, params Int64[] offsets ) where T : struct {
      return ReadAbsolute<T> ( manager, manager.BaseAddress + offset, offsets );
    }
    public static T ReadAbsolute<T> ( ProcessManager64 manager, UInt64 address, params Int64[] offsets ) where T : struct {
      Byte[] Bytes = new Byte[Marshal.SizeOf(typeof(T))];

      // Console.WriteLine($"Reading {typeof(T).Name} at {address:X}, {string.Join(", ", offsets)}");
      foreach ( Int64 offset in offsets )
        address = ReadAbsolute<UInt64>(manager, address) + (UInt64) offset;
    ...
    public static String ReadRelativeUTF8String ( ProcessManager64 manager, UInt64 address, params Int64[] offsets ) {
      return ReadAbsoluteUTF8String ( manager, manager.BaseAddress + address, offsets );
    }
    public static String ReadAbsoluteUTF8String ( ProcessManager64 manager, UInt64 address, params Int64[] offsets ) {
    ...
    public static String ReadString ( ProcessManager64 manager, UInt64 address, UInt32 length ) {
    ...
    public static void WriteRelative<T> ( ProcessManager64 manager, UInt64 offset, T value, params Int64[] offsets ) where T : struct {
      WriteAbsolute<T> ( manager, manager.BaseAddress + offset, value, offsets );
    }
    public static void WriteAbsolute<T> ( ProcessManager64 manager, UInt64 address, T value, params Int64[] offsets ) where T : struct {
